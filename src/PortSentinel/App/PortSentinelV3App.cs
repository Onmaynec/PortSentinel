...
internal sealed class PortSentinelV3App
{
    ...
    [
        ...
        new("3", "BASELINE CENTER", "Создание и сравнение эталонного состояния", ConsoleColor.Magenta),
        new("4", "NETWORK TOOLS", "Live, listeners, connections, processes, scan и updates", ConsoleColor.Green),
        new("5", "STORAGE STATUS", "Пути базы данных и каталога отчётов", ConsoleColor.Yellow),
        new("0", "EXIT", "Безопасно закрыть PortSentinel", ConsoleColor.Red)
    ];

    public PortSentinelV3App(
        Terminal terminal,
    ...
    public async Task RunAsync(CancellationToken token)
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            {
                int index = Array.FindIndex(Menu, x => x.Key[0] == key.KeyChar);
                if (index < 0) continue;
                _selected = index;
    ...
    private void DrawMenu()
    {
        ...
        _terminal.WriteLine($"  v{Program.Version}  •  SESSION INTELLIGENCE UPDATE", ConsoleColor.DarkGray);
    ...
    private Task OpenAsync(string key, CancellationToken token) => key switch
    {
        ...
        "4" => _networkTools.RunAsync(token),
        "5" => StorageStatusAsync(),
    ...
    private async Task RecordLiveAsync(CancellationToken token)
    {
        ...
        while (true)
        {
            ...
            previous = entries.Select(e => e.Identity).ToHashSet(StringComparer.Ordinal);
    ...
    private async Task BaselineCenterAsync(CancellationToken token)
    {
        while (true)
        {
            ...
            _terminal.WriteLine("  [R] Сравнить текущее состояние с «default»", ConsoleColor.Magenta);
            _terminal.WriteLine();
            ...
            else if (key == ConsoleKey.R)
            {
                ...
                BaselineComparison? comparison =
                    await _store.CompareBaselineAsync("default", current, token);
